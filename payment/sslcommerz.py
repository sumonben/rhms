...
def sslcommerz_payment_gateway(request, guest, rooms, amount):
    ...
    try:
        ...
        body = {
            ...
            'success_url': f"http://{request.META.get('HTTP_HOST', 'localhost')}/payment/success/",
            'fail_url': f"http://{request.META.get('HTTP_HOST', 'localhost')}/payment/failed/",
            'cancel_url': f"http://{request.META.get('HTTP_HOST', 'localhost')}/payment/canceled/",
            'ipn_url': f"http://{request.META.get('HTTP_HOST', 'localhost')}/payment/ipn/",
            'emi_option': 0,
